...
export function Zones({ ctx }) {
  const { zones, setZones, saveZones, setToast } = ctx
  ...
  const save = () => {
    saveZones({ ...zones, pace: draft, meta: { ...zones.meta, source: 'Manual edit', updated: new Date().toISOString().slice(0, 10) } })
    setEditing(false); setToast('Zones saved to Supabase')
  }
  ...
  return (
    <div className="animate-rise">
      ...
      <div className="card p-4 mt-4 text-[12.5px] text-muted leading-relaxed">
        <b className="text-ink">How the feed works.</b> Zones are read live from your Supabase project (the <span className="font-mono text-clay">pace_zones</span> table). “Sync from Garmin” re-pulls that table — the place a Garmin worker would write the latest — and edits you make here are saved straight back to Supabase. If the database is ever unreachable it falls back to your saved values and tells you so.
      </div>
    </div>
  )
}
